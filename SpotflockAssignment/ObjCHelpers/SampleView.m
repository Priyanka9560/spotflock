...
//  KLCPopupExample
//
//  Created by Priyanka Bandaru on 23/12/15.
//  Copyright © 2015 Kullect. All rights reserved.
//
...
+ (SampleView*)popupWithContentView:(UIView*)contentView
{
    SampleView* popup = [[[self class] alloc] init];
...
+ (void)dismissAllPopups {
    ...
    for (UIWindow* window in windows) {
        [window forEachPopupDoBlock:^(SampleView *popup) {
            [popup dismiss:NO];
        }];
    }
}
...
//                        finalContainerFrame.origin.y = floorf((CGRectGetHeight(self.bounds) - CGRectGetHeight(containerFrame))/2.0);
                        if (self->_isBridge) {
                            finalContainerFrame.origin.y = [UIScreen mainScreen].bounds.size.height-94;
                        }
                        else {
                            finalContainerFrame.origin.y = floorf((CGRectGetHeight(self.bounds) - CGRectGetHeight(containerFrame))/2.0);
                        }
...
- (void)forEachPopupDoBlock:(void (^)(SampleView* popup))block {
    for (UIView *subview in self.subviews)
    {
        if ([subview isKindOfClass:[SampleView class]])
        {
            block((SampleView *)subview);
        } else {
...
- (void)dismissPresentingPopup {
    ...
    UIView* view = self;
    while (view != nil) {
        if ([view isKindOfClass:[SampleView class]]) {
            [(SampleView*)view dismiss:YES];
            break;
        }
        view = [view superview];
    }
